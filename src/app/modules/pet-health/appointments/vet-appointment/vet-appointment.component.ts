...
export class VetAppointmentComponent {
  ...
  async getPetsByEmail(value: any) {
    ...
    const pets: any = await this.petsServices.filterPetsByHumanIdAndZone(100, 0, "", this.petHuman.id, this.selectedZone);
    if (pets === null) {
      return [];
    }
    return pets;
  ...
  async saveAppointment() {
    ...
    const pet: any = this.pets.find((pet: any) => pet.id === this.myForm.value.petId);
    const vetHuman: any = this.humans.find((human: any) => human.id === this.myForm.value.vetHumanId);
    ...
    const appointment: any = {
      ...
      jsonData: {
        ...
        petHuman: this.petHuman ? {name: this.petHuman.name, email: this.petHuman.email, phone: this.petHuman.phone, nickName: this.petHuman.nickName} : null,
        vetHuman: vetHuman
      }
    };
